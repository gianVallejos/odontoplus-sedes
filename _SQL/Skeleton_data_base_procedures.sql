...
BEGIN
	SET lc_time_names = 'es_ES';
	SELECT dr.nombres, dr.apellidos, FORMAT(IFNULL(SUM((1 - dr.margen_ganancia/100) * (idt.cantidad * ((idt.monto-idt.costo_variable)-(idt.monto*idt.igv/100)))), 0),2) as ingreso
	FROM ingresos_detalle idt
	INNER JOIN doctors dr ON dr.id = idt.doctorId
	WHERE idt.fecha BETWEEN start_date AND end_date
...
BEGIN
	SET lc_time_names = 'es_ES';
  SELECT dr.nombres, dr.apellidos, FORMAT(IFNULL(SUM(dr.margen_ganancia/100 * (idt.cantidad * ((idt.monto-idt.costo_variable)-(idt.monto * (idt.igv/100))))), 0),2) as pago
  FROM ingresos_detalle idt
  INNER JOIN doctors dr ON dr.id = idt.doctorId
	WHERE idt.fecha BETWEEN start_date AND end_date
  AND (xsede_id IS NULL OR idt.sedeId = xsede_id)
