...
class reporteController extends Controller{
    ...
    public function obtenerIngresosMensuales($year){
        $data = DB::select('call OP_Ingresos_get_ingresos_mensuales_anio("'. $year .'")');
        return response()->json(['ingresos' => $data ]);
    }

    public function obtenerEgresosMensuales($year){
        $data = DB::select('call OP_Egresos_get_egresos_mensuales_anio("'. $year . '")');
        return response()->json(['egresos' => $data ]);
    }

    public function obtenerIngresosPacienteFechas($start, $end){
        $data = DB::select('call OP_Ingresos_get_ingresos_por_paciente_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['ingresos' => $data ]);
    }

    public function obtenerNuevosPacientesFechas($start, $end){
        $data = DB::select('call OP_Pacientes_get_pacientes_creados_por_mes_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['nuevos_pacientes' => $data ]);
    }

    public function obtenerPacientesPorCanalFechas($start, $end){
        $data = DB::select('call OP_Pacientes_get_pacientes_por_canal_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['pacientes_canal' => $data ]);
    }

    public function obtenerIngresosPorDoctorFechas($start, $end){
        $data = DB::select('call OP_Ingresos_get_ingresos_por_doctor_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['records' => $data ]);
    }

    public function obtenerPagosPorDoctorFechas($start, $end){
        $data = DB::select('call OP_Pagos_get_pagos_por_doctor_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['records' => $data ]);
    }

    public function obtenerTratamientosFechas($start, $end){
        $data = DB::select('call OP_Tratamientos_get_tratamientos_destacados_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['tratamientos' => $data ]);
    }

    public function obtenerTratamientosPorDoctorFechas($start, $end){
        $data = DB::select('call OP_Tratamientos_get_tratamientos_por_doctor_fechas("'. $start .'", "'. $end .'")');
        return response()->json(['records' => $data ]);
    }
    ...
    public function gananciasFechas($start, $end){
        $ingresos = DB::select('call OP_Ingresos_get_ingresos_por_doctor_doctor_id_fechas("0","'. $start .'","'. $end .'")');
        $totales = DB::select('call OP_Ingresos_get_totales_doctor_id_fechas("0","'. $start .'","'. $end .'")');
